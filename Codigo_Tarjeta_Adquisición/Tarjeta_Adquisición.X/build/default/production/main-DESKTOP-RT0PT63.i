...
void DS18B20_Read(unsigned char *TRIS, unsigned char *PORT, unsigned char *LAT, unsigned char PIN, unsigned char entry){
          ...
          OW_write_byte(0xCC, &TRISA, &LATA, PIN);
          OW_write_byte(0x44, &TRISA, &LATA, PIN);
          ...
          OW_write_byte(0xCC, &TRISA, &LATA, PIN);
          OW_write_byte(0xBE, &TRISA, &LATA, PIN);
...
void i2cwrite2byte (unsigned char add, unsigned char dir, unsigned int data){
   static char si3=0;
    switch (si3){
        ...
        break;
           case 2:
        SSPBUF = (unsigned char)(data & 0x00FF);
        si3=3;
        break;
           case 3:
...
void main(void) {
    ...
    for(unsigned char i=0; i<10; i++) {
        sport[i]= ( EEADR = i, EECON1 &= 0x3F, EECON1bits.RD = 1, EEDATA);
        if(sport[i]>='3') i2cqt++;
    }
    port_config(sport);
    _delay((unsigned long)((100)*(32000000/4000.0)));
    ...
    if(port_save==1){
        for(unsigned char i=0; i<10; i++)
        do{ while (EECON1bits.WR) { continue; } EEADR = (i); EEDATA = (sport[i]); EECON1 &= 0x3F; STATUSbits.CARRY = 0; if (INTCONbits.GIE) { STATUSbits.CARRY = 1; } INTCONbits.GIE = 0; EECON1bits.WREN = 1; EECON2 = 0x55; EECON2 = 0xAA; EECON1bits.WR = 1; EECON1bits.WREN = 0; if (STATUSbits.CARRY) { INTCONbits.GIE = 1; } } while (0);
        port_config(sport);
        _delay((unsigned long)((100)*(32000000/4000.0)));
        port_save=0;
    }
    ...
    if(((meddone & 0x01)>0)){
        ...
        if(sport[7]=='1' && i2busy==0){
           ...
           DS18B20_Read(&TRISA, &PORTA, &LATA, 0x80, 8);
           meddone &= ~0x80 ;
...
# 782 "main.c"
    ...
    switch(i2cstep)
     {
         case 0:
         i2cwrite2byte (0x2A<<1, 0x00,0x2706);
         break;
         case 1:
