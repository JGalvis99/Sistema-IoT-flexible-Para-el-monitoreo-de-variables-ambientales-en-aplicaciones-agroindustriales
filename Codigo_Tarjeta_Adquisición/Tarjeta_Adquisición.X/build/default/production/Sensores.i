...
unsigned char check_response(unsigned char port, unsigned char tris, unsigned char TOUT)
 {
   ...
   TMR2ON = 1;
   if(tris==0){
   while (!(PORTA & port) && !TOUT);
   if (TOUT)
      return 0;
   else
    { TMR2 = 0;
      while (((PORTA & port)>0) && !TOUT);
      if (TOUT)
         return 0;
      else
      { TMR2ON = 0;
...
void DHT22READ (unsigned char port, unsigned char tris, unsigned char entry, unsigned char TOUT, unsigned char dhtresult[], unsigned char COM[])
{
    ...
    if (!check)
       ...
       {
         dhtresult[0] = read_byte(port, tris);
         dhtresult[1] = read_byte(port, tris);
         dhtresult[2]= read_byte(port, tris);
         dhtresult[3] = read_byte(port, tris);
         COM[1]=entry + 48;
      }
}
